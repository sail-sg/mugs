...
class Attention(nn.Module):
    def __init__(
        ...
        self.qkv = nn.Linear(dim, all_head_dim * 3, bias=qkv_bias) #False)
        # if qkv_bias:
...
def get_sinusoid_encoding_table(n_position, d_hid, cls=True): 
    ...
    if cls:
        sinusoid_table = np.concatenate([np.zeros((1, d_hid)), sinusoid_table])
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 use_abs_pos_emb=True, use_learned_abs_pos_emb=True,
        ...
        if use_abs_pos_emb and use_learned_abs_pos_emb:
        ...
        if self.pos_embed is not None and use_learned_abs_pos_emb:
