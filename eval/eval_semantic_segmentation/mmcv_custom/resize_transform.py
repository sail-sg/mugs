...
class CenterCrop(PIPELINES.get('RandomCrop')):
    ...
    def get_crop_bbox(self, img):
        ...
        offset_h = (max(img.shape[0] - self.crop_size[0], 0) + 1) // 2
        offset_w = (max(img.shape[1] - self.crop_size[1], 0) + 1) // 2
        crop_y1, crop_y2 = offset_h, min(offset_h + self.crop_size[0], img.shape[0])
        crop_x1, crop_x2 = offset_w, min(offset_w + self.crop_size[1], img.shape[1])
