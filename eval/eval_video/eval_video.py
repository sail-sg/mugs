...
"""
Video Segmentation evaluation implementation.
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pretrained_weights', default='./vit_small_800_best/checkpoint.pth', type=str, help="Path to pretrained weights to evaluate.")
    parser.add_argument('--arch', default='vit_small', type=str,
        choices=['vit_tiny', 'vit_small', 'vit_base'], help='Architecture (support only ViT atm).')
    parser.add_argument('--patch_size', default=16, type=int, help='Patch resolution of the model.')
    parser.add_argument("--checkpoint_key", default="teacher", type=str, help='Key to use in the checkpoint (example: "teacher")')
    parser.add_argument('--output_dir', default="./small_video_segmentation", help='Path where to save segmentations')
    parser.add_argument('--data_path', default='/mnt/aiops/common/DAVIS2017_video/DAVIS', type=str)
    ...
    parser.add_argument("--bs", type=int, default=8, help="Batch size, try to reduce if OOM")
    args = parser.parse_args()

    print("git:\n  {}\n".format(utils.get_sha()))
    print("\n".join("%s: %s" % (k, str(v)) for k, v in sorted(dict(vars(args)).items()))) 
    args.output_dir = './%s/topk-%d-n-frames-%d-mask-neighbors-%d'%(args.output_dir, args.topk, args.n_last_frames, args.size_mask_neighborhood)
